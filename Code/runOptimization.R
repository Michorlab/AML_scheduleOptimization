...
# Get concentration for a given dose and time interval
...
# Funtion estimates cell count during and end at trial
...
# Function that runs one trial for a specific LSD1 and 6-MP dose schedule
optimFunc = function(doses, hour, taus, L_missed, M_OnOff, init, cumulative = T){
...
### Function saves output for any given schedule of LSD1 and 6-MP
# file: name of file to save
# hour: length of trial in hours
# taus: time interval between doses in hours
# L_OnOff: a vector of size 2, with first element being the number of active doses and
#           second element is number of holidays of LSD1. Total should be 7
# L_OnOff: a vector of size 2, with first element being the number of active doses and
#           second element is number of holidays of 6-MP. Total should be 7
# NOTE: initial cell count is 100 undifferentiated cells and 0 differentiated cells.
saveOutput = function(file, hour, taus, L_OnOff = c(7,0), M_OnOff = c(7,0)){
  if (sum(L_OnOff) != 7){
  ...
  itFunc = function(it, init){
    optimFunc(doses = c(L = doses$L[it], M = doses$M[it]), L_missed = L_missed, 
              M_OnOff = M_OnOff, hour = hour, taus = taus, init = init, cumulative = F)
  }
  ...
  fileName = paste0("../Output_200718/", file, ".csv")
  write_csv(x = doses, path = fileName, col_names = T)
}

# ########## CONSTANT CONCENTRATION (No PK) ##########
...
#   L = concs[1]
#   M = concs[2]
#   # Don't uncomment the following chunk
#   # if(L < .025){
